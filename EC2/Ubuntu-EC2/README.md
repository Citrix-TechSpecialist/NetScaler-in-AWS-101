...
```
sudo docker run -dt -p 8080:3000 --name=drop-share-1 --restart=always -v /tmp:/data psitrax/psitransfer
```
...
In this section we will simply launch two web sites on the Dockerhost that are labeled as *WebServer-A* and *WebServer-B* to serve as additional examples of backend services that can potentially be targets for load balancing when [deploying NetScaler VPX](https://github.com/Citrix-TechSpecialist/NetScaler-in-AWS-101/tree/master/EC2/Deploy-NS).
...
5. Launched sample backed HTTP websites
