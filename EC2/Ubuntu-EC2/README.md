...
So far you should have deployed a [Windows 2016 EC2 instance](../Windows-EC2/README.MD) that we can RDP into. However for hosted services, backend servers are often Linux based compute servers. In this section we will deploy a Linux Instance on a private subnet that has  outbound access to the internet for updates and apt-packages but no direct access for clients from the internet. We will use the Windows Machine as a [bastion host](https://aws.amazon.com/blogs/security/controlling-network-access-to-ec2-instances-using-a-bastion-server/) to SSH into this instance and configure some web hosted services that we can access only within our Virtual Private Network. Later when we [deploy NetScaler ADC](../Deploy-NS/README.md), we will set up external access in a private back-end subnet via our NetScaler reverse proxy.

Lets begin by launching an Ubuntu instance, similar to how we did for our Windows 2016 server. 
...
* **Network Interfaces** by default create a single vNIC for the instance (eth0). The first and default interface on the instance is it's "default" [Elastic Network Interface (ENI)](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-eni.html). During this step you can specify a static private IP within the subnet or allow an IP to be allocated based on DHCP. 
  > Enter 172.16.20.10 under Primary IP for a static private IPv4 address, but note you can add additional private IPs that the instance can own associated with a particular ENI. You will also see later when we deploy NetScaler ADC, we will manage ENI private IPs that will be used as VIPs for Load Balancing. 
...
Here we associate block storage to the instances where all filesystem, OS, and persistent data will be stored for the EC2 instance. These volumes are known as [EBS volumes](https://aws.amazon.com/ebs/details/) in AWS. EBS can be elastic in relation to their storage size, IOPs, and Encryption. 
  > Enter 8GB for size and General Purpose for Volume Type for our use case. 
...
Now that our Linux Instance is up and running, we will now attempt to SSH from our Windows 2016 Server instance. This will be inter-VPC network traffic where the request will originate from our **Client Subnet** that is destined to our **Server Subnet** within our **Demo VPC**
...
Lastly, do an update and upgrade on the host to get everything up to latest versions with the following command: 
...
Also, to check the storage available to you on the sharemount, compare the available storage space on your `/` (root) directory and compare it to the space available on the mount point `/data`. Enter the following to see: 
...
Now that we've got our network storage mount established and all of our pre-requisites complete, we will host a simple *Dropbox* like file sharing webserver that will store data in the EFS volume mount. We will then upload a file or two into the webapp and it will store the files data in the EFS volume mount. 
...
Begin by launching a [*psi-transfer* docker container](https://hub.docker.com/r/psitrax/psitransfer/) that provides a web UI to upload and download files to and from a mapped volume mount. Enter the following command in the SSH session: 
...
* The following `docker run` command will run a container in the background with the `-dt` flag.
...
* In the command we will map a share volume with the `-v` flag where the container can store data in it's `/data` container-local directory that is mapped to the `/data` host-local directory. Basically any data saved in the `/data` directory within the container by the application will be backed on the EFS mount `/data` on the ubuntu host.

* The container will be based on the [`psitrax/psitransfer`](https://hub.docker.com/r/psitrax/psitransfer/) image that is publicly available from [docker hub](https://docs.docker.com/docker-hub/). 
...
In this section we will simply launch two web sites on the Dockerhost that are labeled as *WebServer-A* and *WebServer-B* to serve as backend services to load balance when we [deploy a NetScaler VPX](../Deploy-NS) for a simple load balancing exercise. 
...
Within your RDP session, navigate to `http://172.16.20.10:9090` and `http://172.16.20.10:9091` to validate the websited are up, running, and available. 

> **NOTE:** Please remember to open ports 9090 and 9091 on your security group associated with the Ubuntu Server's ENI otherwise you will not be able to resolve the websites. *Allow a custom TCP Rule on port 9090 and 9091 from any source IP.*
